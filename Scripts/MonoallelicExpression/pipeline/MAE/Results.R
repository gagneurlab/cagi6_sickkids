...
res <- merge(res, ot[, .(SYMBOL, OMIM, OMIM_Inheritance)], 
             by.x = 'gene_name', by.y = 'SYMBOL', all.x = T, sort = F)

# Calculate variant frequency within cohort 
maxCohortFreq <- snakemake@config$mae$maxVarFreqCohort
...
fwrite(res[MAE == TRUE], snakemake@output$res_signif, 
