...
#'    - Sample_Anno: 'Data/project_data/sample_annotation.tsv'
#'  output:
...
melt_mat <- merge(melt_mat, sa[, .(DNA_ID, Cohort, WGS_technology)], all.x = T, by.x = 'Var1', by.y = 'DNA_ID')

ggplot(melt_mat, aes(value)) + geom_histogram(aes(fill = WGS_technology), bins = 25) + 
  theme_bw(base_size = 14) + scale_fill_ptol() +
  labs(x = 'Proportion of matching DNA-RNA variants', y = 'DNA-RNA sample combinations') + 
